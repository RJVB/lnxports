...
variant universal {
    ...
    configure {
        ...
        foreach arch ${universal_archs_to_use} {
            ui_msg "$UI_PREFIX [format [msgcat::mc "  Configuring %1\$s for architecture %2\$s"] ${subport} ${arch}]"
    ...
    build {
        ...
        foreach arch ${universal_archs_to_use} {
            ui_msg "$UI_PREFIX [format [msgcat::mc "  Building %1\$s for architecture %2\$s"] ${subport} ${arch}]"
            set muniversal.current_arch ${arch}
    ...
    destroot {
        ...
        foreach arch ${universal_archs_to_use} {
            ui_msg "$UI_PREFIX [format [msgcat::mc "  Staging %1\$s into destroot for architecture %2\$s"] ${subport} ${arch}]"
            set muniversal.current_arch ${arch}
    ...
    test {
        foreach arch ${universal_archs_to_use} {
            ...
            if { (${os.arch}=="i386" && ${arch}!="ppc64") || (${os.arch}=="powerpc" && ${arch}!="i386" && ${arch}!="x86_64") } {
                ui_msg "$UI_PREFIX [format [msgcat::mc "  Testing %1\$s for architecture %2\$s"] ${subport} ${arch}]"
